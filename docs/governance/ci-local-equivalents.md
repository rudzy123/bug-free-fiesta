...
| Playwright e2e                       | `pnpm --filter @esign/web exec playwright install chromium` then `pnpm test:e2e` (starts Compose, migrates, seeds, API, worker, and Next.js). Optional: `pnpm --filter @esign/web exec playwright test --grep @smoke`   |
