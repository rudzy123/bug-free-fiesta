...
Capstone for the v1 architecture. Application packages and apps implement these decisions; this file remains the index of ADRs and residual risk. This file does **not** claim ESIGN, UETA, eIDAS, HIPAA, SOC 2, ISO 27001, or other compliance.
...
| PDFs in private object storage                     | [0004](adrs/0004-private-object-storage.md)                | Short DB transactions; MinIO locally; S3 in production |
...
| External audit checkpoint anchoring                | [0015](adrs/0015-audit-checkpoint-anchoring.md)            | Hash chain is not DBA-proof; WORM is separate          |
...
Priority is **P0** (must mitigate before handling real customer documents), **P1** (mitigate early), **P2** (track; may accept with owner). Residual risks remain after intended controls. None of these rows is a legal opinion. Align open engineering items with [security reviews](../security/reviews/).
...
| R3  | P0       | Raw signing tokens in logs or DB        | Universal stolen links              | Hash at rest (HMAC pepper in prod); logger deny list  | Downstream log pipeline; pepper rotation    | Engineering + ops          |
| R4  | P0       | Public object-storage bucket            | Mass PDF leak                       | Private IaC; `s3` driver in production                | Console misclick                            | Ops                        |
| R5  | P0       | Duplicate finalization / split artifact | Two “originals”                     | Lease + unique digest                                 | Orphan objects                              | Engineering                |
| R6  | P0       | Malicious PDF in worker                 | RCE or SSRF                         | Structural inspect; no fetches; timeouts; pin pdf-lib | Library 0-day; not commercial AV            | Engineering                |
...
| R11 | P1       | Audit chain break or UPDATE             | Integrity failure                   | Insert-only role; verifier; checkpoints; runbook      | Superuser can still rewrite the whole chain | Security + ops             |
| R12 | P1       | CSRF/XSS on account session             | Void or send as owner               | SameSite, CSRF, CSP, HTTP-only cookies                | New HTML sinks; style-src unsafe-inline     | Engineering                |
| R13 | P1       | Oversized payload DoS                   | Outage                              | Body/field/quota limits                               | Volumetric DDoS; in-process rate limits     | Engineering + ops          |
| R14 | P1       | Forged forwarded headers                | Poisoned IP/tenant                  | BFF does not copy client XFF; TRUST_PROXY hops        | Edge misconfig                              | Ops                        |
...
| R17 | P2       | Backup theft or rewrite                 | Historical leak/tamper              | Encrypted backups; restore control                    | Cloud admin; drill not yet proven           | Ops + **legal review**     |
| R18 | P2       | Dependency compromise                   | Supply-chain malware                | Lockfile, few deps, CI pins, image Trivy              | Transitive packages                         | Engineering                |
...
| R21 | P2       | Shared-tenancy noisy neighbor           | Degraded signing                    | Quotas, rate limits                                   | No silo / no RLS in v1                      | Engineering                |
...
4. Data residency and subprocessors (email, object storage, optional AV).
...
- Track deferred security items (SEC-008, SEC-011, SEC-012, SEC-019, SEC-022) as decision-owned work, not silent “TODO in code”.
...
- Keep [production-readiness checklist](../deployment/production-readiness-checklist.md) current when config contracts change.
