...
function authorize(deps: { authorization: AuthorizationPolicy }, actor: RequestActor, resource: {
...
async function verifyFinalizedArtifact(
  ...
  if (stored === null) {
    const fallback = await deps.storage.getObject({
      organizationId: input.organizationId,
    ...
    if (deps.hashing.sha256Hex(fallback.body) !== artifact.sha256Digest) {
      failures.push(failure('ARTIFACT_DIGEST_MISMATCH', input, event));
    }
    return;
  }
  if (deps.hashing.sha256Hex(stored.body) !== artifact.sha256Digest) {
    failures.push(failure('ARTIFACT_DIGEST_MISMATCH', input, event));
  }
}
...
export function createVerifyAuditChain(deps: {
  ...
  return async function verifyAuditChain(
    ...
    const events = [...(await deps.auditLogs.listByDocument({
      organizationId: input.organizationId,
      documentId: input.documentId,
    }))];
    ...
    if (sequences.some((sequence, index) => sequence !== index)) {
      if (sequences.some((sequence, index) => index > 0 && sequence < (sequences[index - 1] ?? 0))) {
        ...
        failures.push(failure('SEQUENCE_GAP', input, events.find((event, index) => event.sequence !== index) ?? null));
      }
    }
...
export function createVerifyOrganizationAuditChains(deps: Parameters<typeof createVerifyAuditChain>[0]) {
  ...
  return async function verifyOrganizationAuditChains(input: {
    ...
    if (input.actor !== undefined && input.actor.type === 'account_user') {
      deps.authorization.assertAllowed(input.actor, 'audit.verify', {
        organizationId: input.organizationId,
      });
    }
    const documents =
...
function dedupeFailures(
  failures: readonly AuditVerificationFailure[],
): AuditVerificationFailure[] {
